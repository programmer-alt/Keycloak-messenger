import { Request, Response, Router } from "express";
import { body, validationResult } from "express-validator";
import { saveMessage, clearUserMessages } from "../utilsComponents/messageService";
import pool from "../db";

/**
 * Маршруты для работы с сообщениями
 *
 * Этот модуль определяет маршруты для отправки и получения сообщений.
 *
 * POST / - Отправка сообщения:
 *  - Проверяет наличие обязательных полей: receiver_id и message.
 *  - Извлекает id отправителя из токена.
 *  - Добавляет сообщение в базу данных и возвращает результат.
 *  - Обрабатывает ошибки валидации и серверные ошибки.
 *
 * GET / - Получение сообщений:
 *  - Извлекает id пользователя из токена.
 *  - Получает все сообщения пользователя из базы данных, сортируя их по дате.
 *  - Возвращает список сообщений или обрабатывает серверные ошибки.
 */
const router = Router();

// Отправка сообщения
router.post(
  "/",
  [
    body("receiver_id")
      .isString()
      .notEmpty()
      .withMessage("receiver_id обязательно"),
    body("message").isString().notEmpty().withMessage("message обязательно"),
  ],
  async (req: Request, res: Response): Promise<void> => {
    try {
      // получаем id пользователя из токена
    // Используем sub (UUID) для идентификации отправителя
    const senderId: string = (req as any).kauth.grant.access_token.content.sub;
      // получаем id получателя и текст сообщения
      const { receiver_id, message } = req.body;

      if (!receiver_id || !message) {
        res.status(400).json({ error: "receiver_id и message обязательны" });
        return;
      }
      // добавляем сообщение в базу через сервис
      const savedMessage = await saveMessage(senderId, receiver_id, message);

      res.status(201).json(savedMessage);
    } catch (error) {
      console.error("Ошибка при отправке сообщения:", error);
      res.status(500).json({ error: "Ошибка сервера" });
    }
  }
);

// Получение сообщений пользователя
router.get("/", async (req: Request, res: Response): Promise<void> => {
  try {
    // получаем id пользователя из токена
    // Используем sub (UUID) для идентификации пользователя
    const userId: string = (req as any).kauth.grant.access_token.content.sub;
    // получаем все сообщения пользователя из базы данных
    const query = `
      SELECT * FROM messages
      WHERE sender_id = $1 OR receiver_id = $1
      ORDER BY created_at DESC;
    `;
    // выполняем запрос в базе данных и возвращаем результат
    const result = await pool.query(query, [userId]);

    res.json(result.rows);
  } catch (error) {
    console.error("Ошибка при получении сообщений:", error);
    res.status(500).json({ error: "Ошибка сервера" });
  }
});

// Очистка всех сообщений пользователя
router.delete("/", async (req: Request, res: Response): Promise<void> => {
  try {
    // получаем id пользователя из токена
    // Используем sub (UUID) для идентификации пользователя
    const userId: string = (req as any).kauth.grant.access_token.content.sub;

    // очищаем все сообщения пользователя через сервис
    const deletedCount = await clearUserMessages(userId);

    res.json({ success: true, message: `Удалено ${deletedCount} сообщений` });
  } catch (error) {
    console.error("Ошибка при очистке сообщений:", error);
    res.status(500).json({ error: "Ошибка сервера" });
  }
});

export default router;
